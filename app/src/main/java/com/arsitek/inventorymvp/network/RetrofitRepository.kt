...
class RetrofitRepository(private val context: Context) {
    companion object {
        private val TAG = RetrofitRepository::class.java.canonicalName
    }

    fun getDataLogin(username: String, password: String,callback: RepositoryCallback<ResponseLogin>) {
        val init =
            Initretrofit().getInstance().userLogin(username, password)
        Log.d("kirim", username)
        init.enqueue(object : Callback<ResponseLogin> {
            override fun onResponse(call: Call<ResponseLogin>, response: Response<ResponseLogin>) {
                val res = response.body()
                Log.d("getdatalogin", res?.status.toString())
                if (res?.status == 200) {
